...
clustering_df = pd.DataFrame({
    'Title': list(clustering_coefficient.keys()),
    'Clustering Coefficient': list(clustering_coefficient.values())
})

# Filter top 15
top_15_clustering = clustering_df.sort_values(by='Clustering Coefficient', ascending=False).head(15)

# Save to CSV
top_15_clustering.to_csv('top_15_clustering_coefficient_citations_results.csv', index=False)

# Visualization
titles = top_15_clustering['Title'].tolist()
G_subgraph = G_undirected.subgraph(titles)
pos = nx.spring_layout(G_subgraph, seed=42)
plt.figure(figsize=(15, 12))

node_size = [15000 * clustering_coefficient[node] for node in G_subgraph.nodes()]
nx.draw_networkx_nodes(G_subgraph, pos, node_size=node_size, node_color='orange', alpha=0.7)
...
plt.title("Citation Network - Clustering Coefficient (Top 15)", size=20)
plt.axis('off')
plt.savefig('clustering_coefficient_citations_visualization.png')
plt.show()